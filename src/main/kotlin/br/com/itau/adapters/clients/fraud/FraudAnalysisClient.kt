...
class FraudAnalysisClient(
    ...
    override fun analyzeFraud(customerId: String): FraudAnalysisResult {
        ...
        logger.info("Calling fraud analysis API for customer: {}", customerId)
        ...
        try {
            ...
            logger.warn("Customer not found in fraud system: {}", customerId)
            throw FraudAnalysisException("Customer not found: $customerId", e)
        } catch (e: HttpServerErrorException) {
            logger.error("Fraud analysis API error for customer: {}", customerId, e)
            throw FraudAnalysisException("Fraud analysis service unavailable", e)
        } catch (e: Exception) {
            logger.error("Unexpected error calling fraud analysis API", e)
